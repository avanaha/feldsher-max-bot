...
// Удаляем старые базы с кривым форматом дат
const oldDbs = ['database.sqlite', 'bot.db'];
for (const oldDb of oldDbs) {
  const oldPath = join(dataDir, oldDb);
  if (existsSync(oldPath)) {
    try {
      unlinkSync(oldPath);
      console.log(`Removed old database: ${oldDb}`);
    } catch (e) {}
  }
}

// Устанавливаем путь к новой базе
process.env.DATABASE_URL = `file:${join(dataDir, 'feldsher.db')}`;
...
async function createBackup() {
  try {
    const dbPath = join(dataDir, 'feldsher.db');
    ...
    const backupPath = join(dataDir, 'backups', `backup-${timestamp}.db`);
